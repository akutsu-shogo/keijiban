...
<body>
	...
	$texts_delete = file($filename);
		...
		$texts_delete_element = explode("<>" , $value_delete);
			...
			if ($texts_delete_element[0] == $_POST["delete"]){
				...
				if($texts_delete_element[4] == $_POST["pass_word_delete"]){
	...
	$edit_comment = "";
		...
		$texts_edit_element = explode("<>" , $value_edit);
			...
			if($texts_edit_element[0] == $edit){
				...
				if($texts_edit_element[4] == $_POST["pass_word_edit"]){
	...
	$texts_edit = file($filename);
		...
		$text_edit = explode("<>" , $value_edit);
			...
			if($text_edit[0] == $_POST["number"]){
				...
				if($text_edit[4] == $_POST["pass_word"]){
	...
	if(!empty($_POST["name"]) && !empty($_POST["comment"] && !empty($_POST["pass_word"]) && empty($_POST["number"] ))){
		...
		$number = $i."<>".$name."<>".$comment."<>".$date."<>".$pass_word."<>";
		$fp = fopen($filename , "a");
		fwrite($fp , $number."\n");
		fclose($fp);
	}
